...
class MonthEvaluationOpportunityLossOnHold(SimulationTask):
    """
    ...
    def compute_reward(self, action: np.ndarray, info: Dict[str, Any]) -> np.ndarray:
        ...
        if action == 0.0:
            ...
            if np.isclose(current_crypto_capital, [0.0], atol=1e-3):
                opportunity_loss += self._compute_max_opportunity_loss()
        else:
            ...
            opportunity_loss += self._compute_max_opportunity_loss()
            if np.isclose(current_capital, [0.0], atol=1e-3):
                opportunity_loss += self._compute_max_opportunity_loss()
